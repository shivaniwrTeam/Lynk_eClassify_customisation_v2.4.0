...
//This will create delay so we don't face rapid api call
    ...
    return BlocBuilder<SearchItemCubit, SearchItemState>(
      builder: (context, state) {
        ...
        if (state is SearchItemSuccess) {
          ...
          int total = state.searchedItems.length;
              ...
              children: [
                ...
                ...[
                  for (int i = 0; i <= total; i += gridCount) ...[
                    _buildSearchListViewSection(
                      context,
                      i,
                      min(gridCount, total - i),
                      state.searchedItems,
                    ),
                    if (total - i - gridCount > 0)
                      Container(
                          alignment: AlignmentDirectional.center,
  ...
  Widget _buildSearchListViewSection(BuildContext context, int startIndex,
              ...
              children: [
                ...
                ...[
                  for (int i = 0; i <= total; i += gridCount) ...[
                    ...
                    if (total - i - gridCount > 0)
                      Center(child: NativeAdWidget(type: TemplateType.medium)),
                  ],
                ],
                if (state.isLoadingMore)
                  Center(
